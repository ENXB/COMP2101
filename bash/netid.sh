...
###If the number of command line options is 1 and that option is "-v", enable verbose mode.
###If it's not, set the interface variable to be the value of the option.
if [ "$#" = 1 ] && [ "$1" = "-v" ]; then
  verbose="yes"
elif [ "$#" = 1 ] && [ "$1" != "-v" ]; then
  interface="$1"
### If the number of command line options is 2, check if one of them is -v, if so, enable verbose mode and the other option will become the interface variable.
elif [ "$#" = 2 ] && [ "$1" = "-v" ]; then
  verbose="yes"
  interface="$2"
elif [ "$#" = 2 ] && [ "$2" = "-v" ]; then
...
default_router_address=$(ip r s default metric 100| cut -d ' ' -f 3)
...
### If the interface variable is empty. find a list of non-loopback interfaces and assign them to the interfaces variable.
### If it is not empty, assign its value to the interfaces variable.
[ -z $interface ] && interfaces=$(ip link | grep BROADCAST | awk -F: '{print $2}' ) || interfaces=$interface
###Generate a report for each interface listed in interfaces.
